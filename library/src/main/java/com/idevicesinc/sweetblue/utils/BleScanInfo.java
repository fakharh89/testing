...
public final class BleScanInfo implements UsesCustomNull
{
    ...
    public BleScanInfo(BleScanRecord scanInfo)
    {
        ...
        m_serviceUuids = new ArrayList<>();
        addServiceUUIDs(scanInfo.getServiceUUIDS());
        m_manufacturerDataList = scanInfo.getManufacturerDataList();
        if (m_manufacturerDataList != null && m_manufacturerDataList.size() > 0)
        {
            ManufacturerData data = m_manufacturerDataList.get(0);
            m_manufactuerId = data.m_id;
        ...
        m_serviceData = scanInfo.getServiceData();
        m_localName = scanInfo.getName();
        m_shortName = scanInfo.isShortName();
        m_completeUuidList = scanInfo.isCompleteUuidList();
    }
